...
const App = () => {
  ...
  const handleRestart = () => {
    ...
    setRestart((prev) => !prev); // Force reset of state
  };
  ...
  return (
    <div className={`container ${theme}`}>
      ...
      <div>
        <select onChange={(e) => handleStartTest(Number(e.target.value))} defaultValue={1}>
...
const InputBox = ({ text, timer, setTimer, restart, testDuration, onRestart }) => {
  ...
  const getVisibleText = () => {
    return text.substring(currentIndex, currentIndex + visibleTextLength).replace(/ /g, '\u00A0');
  };
